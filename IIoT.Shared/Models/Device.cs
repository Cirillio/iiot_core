...
public record Device
{
    ...
    /// <summary>
    /// Максимальный диапазон адресов в одном групповом запросе для регистров (16-бит слова).
    /// Ограничивает ширину чанка при группировке Holding/Input регистров.
    /// </summary>
    public int MaxRegisterSpan { get; init; } = 120;

    /// <summary>
    /// Максимальный диапазон адресов в одном групповом запросе для битовых таблиц (Coil/DiscreteInput).
    /// Отдельный лимит, т.к. биты можно читать пачками до 2000 — урезать их по MaxRegisterSpan нерационально.
    /// </summary>
    public int MaxBitSpan { get; init; } = 2000;

    /// <summary>
    /// Флаг активности устройства (намерение оператора).
    /// Если false, устройство исключается из цикла опроса. По умолчанию: true.
    /// НЕ изменяется автоматически — это конфиг, а не рантайм-состояние.
    /// </summary>
    public bool IsActive { get; init; } = true;

    /// <summary>
    /// Рантайм-флаг доступности: смог ли коллектор сейчас связаться с устройством.
    /// Пишется коллектором (с гистерезисом), отделён от IsActive.
    /// </summary>
    public bool IsOnline { get; init; }

    /// <summary>
    /// Время последнего успешного контакта с устройством (UTC).
    /// </summary>
    public DateTime? LastSeen { get; init; }

    /// <summary>
    /// Текст последней ошибки связи (если устройство недоступно).
    /// </summary>
    public string? LastConnError { get; init; }
